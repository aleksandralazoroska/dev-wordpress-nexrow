	...
	function posts_grid_shortcode( $atts, $content = null, $shortcodename = '' ) {
		...
		switch ($link) {
			...
			foreach ( $posts as $j => $post ) {
				...
				$prettyType     = 0;
				$advantages  = strpos($custom_class, "advantages");
				$without_zoom  = strpos($custom_class, "without-image-zoom");
				...
				$output .= '<li class="'. $spans .' list-item-'.$count.'">';
					...
					if ($custom_class && $advantages !== false) {
						$output .= '<div class="desc top"><div>';
					...
					if ($custom_class && $without_zoom !== false) {
					...
					$output .= '<div class="desc">';
												if ($advantages == false) {							$output .= '<h5>';								$output .= get_the_title($post_id);							$output .= '</h5>';						}
						if ($meta == 'yes') {
							// begin post meta
